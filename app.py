...
CHUNK_SIZE = 1000
CHUNK_OVERLAP = 100
# The previous experimental model was retired by the Gemini API.
# Use Google's stable text-embedding model instead.
...
def get_ollama_models():
    """Return models exposed by the local Ollama service."""
    try:
...
def handle_sidebar():
    ...
    provider = st.sidebar.selectbox(
        "AI Provider",
        ["Google Gemini (API)", "On-device (Ollama)"],
        help="Use Ollama to answer and index documents locally without a Gemini API key.",
    )
    ...
    if provider == "Google Gemini (API)":
        ...
        st.sidebar.success("✅ Local mode: no Google API key or quota is used")
    ...
    if provider == "Google Gemini (API)":
        ...
        selected_model = st.sidebar.selectbox(
            "Local Ollama model",
            get_ollama_models(),
            help="Models installed in Ollama. Start Ollama before using local mode.",
        )
        st.sidebar.caption(
            f"Uses local embeddings: {LOCAL_EMBEDDING_MODEL}. The first run downloads this small embedding model."
        )
    ...
    return provider, selected_model, uploaded_file, api_key


def handle_document_processing(uploaded_file="", provider="Google Gemini (API)"):
    if st.button("🚀 Process Document", type="primary"):
        user_api_key = st.session_state.get("api_key", "")
        if provider == "Google Gemini (API)" and not user_api_key:
...
@st.cache_resource()
def get_local_chat_model(model_name: str):
    """Return a local Ollama model; Ollama must be running on this computer."""
    return Ollama(model=model_name, base_url=OLLAMA_URL, temperature=0)
...
configure_page()
center_app()
provider, selected_model, uploaded_file, user_api_key = handle_sidebar()
if uploaded_file:
    st.success(f"✅ File uploaded: {uploaded_file.name}")
    st.info(f"📊 Size: {uploaded_file.size} bytes")

handle_document_processing(uploaded_file, provider)
chat_model = None
if provider == "On-device (Ollama)":
    chat_model = get_local_chat_model(selected_model)
elif user_api_key:
...
if chat_model is None:
    st.warning(
        "Please enter your Google Gemini API key, or select On-device (Ollama), to start chatting."
    )
