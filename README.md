...
Maintained by **Abhishek Pal** — dev.04.palabhishek@gmail.com
...
Try it out here: [Document RAG Assistant Live Demo](https://00-abhishek-notemind-document-qa-rag-system-app-codexrel-qwfhxy.streamlit.app/)
...
### On-device mode (Ollama)

The sidebar has an **On-device (Ollama)** provider option. It automatically lists
models installed in Ollama (including `qwen2.5-coder:32b` when available) and uses
that model for answers. Document search uses the local
`sentence-transformers/all-MiniLM-L6-v2` embedding model, so this mode does not use
a Google API key or Gemini quota. The embedding model downloads once on its first
use, then is cached locally.

#### Using Qwen locally

1. Ensure Ollama is running and that the model is installed (for example, `qwen2.5-coder:32b`).
2. Start the app with `uv run streamlit run app.py`.
3. In the sidebar, select **On-device (Ollama)** under **AI Provider**.
4. Select an installed model from **Local Ollama model**, upload a document, and process it.

This mode avoids Gemini API quotas. It uses Ollama for answers and a local embedding model for document retrieval. The first document-processing run may take longer while the embedding model is downloaded and cached.
...
**Abhishek Pal**  
Email: dev.04.palabhishek@gmail.com
